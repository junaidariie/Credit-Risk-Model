...
The final Logistic Regression evaluation was performed at the selected probability threshold of **`0.85`** using the experiment notebook. These are the final evaluation figures reported for the project.
...
With the default classification threshold of **`0.50`**, the Logistic Regression model produced:
...
The threshold was moved from `0.50` to **`0.85`** to obtain a more balanced precision-recall operating point for the default class rather than relying on the conventional `0.50` cutoff.

> **Evaluation note:** The threshold sweep was used to study the precision-recall trade-off, and `0.85` was selected as the operating threshold because it provided a substantially more balanced result than the default `0.50` cutoff. The final classification report below reflects the selected threshold evaluation from the notebook.
...
The final model uses **Logistic Regression** because it provides strong performance with better interpretability and direct compatibility with the scorecard framework.

A **threshold of `0.85`** is used to convert predicted default probabilities into binary predictions for the final evaluation.
